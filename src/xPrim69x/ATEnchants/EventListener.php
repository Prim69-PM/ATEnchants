...
class EventListener implements Listener{
	...
	public function onArmorChange(EntityArmorChangeEvent $event){
		...
		foreach ($ef as $effect => $enchant) {
			if ($item->hasEnchantment($enchant) && !$item->equals($old, false, true)) {
			...
			if ($old->hasEnchantment($enchant) && !$old->equals($item, false, true)) {
				if ($player->hasEffect($effect)) {
					$player->removeEffect($effect);
				}
			}
		}
		if ($item->hasEnchantment(82) && !$item->equals($old, false, true)) {
			$enchantmentLevel = $item->getEnchantment(82)->getLevel();
			$player->setMaxHealth($player->getMaxHealth() + ($enchantmentLevel * 2));
		}
		if ($old->hasEnchantment(82) && !$old->equals($item, false, true)) {
